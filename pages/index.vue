<template>
  <div v-if="Message" :class="Message.type === 'error'
    ...
    <button type="button" :class="Message.type === 'error'
    ...
    <div class="relative w-full max-w-md max-h-full">
      ...
      <div class="relative rounded-lg shadow bg-black overflow-hidden">
        <div v-if="Loading" role="status"
          ...
          <svg aria-hidden="true" class="w-8 h-8 mr-2 animate-spin text-gray-600 fill-fuchsia-900" viewBox="0 0 100 101"
        ...
        <div v-if="Loading" class="absolute top-0 left-0 right-0 bottom-0 bg-black/70 z-[99999]"></div>
        ...
        <div class="px-6 py-6 lg:px-8">
          ...
          <form ref="FormCommander" @submit="Commander" class="space-y-6" action="#">
            <div>
              <label for="nom_complet" class="block mb-2 text-sm font-medium text-white">Nom complet</label>
              <input v-model="Order.nom_complet" type="nom_complet" name="nom_complet" id="nom_complet"
            ...
            <div>
              ...
              <input v-model="Order.email" type="email" name="email" id="email"
                class="border text-sm rounded-lg block w-full p-2.5 outline-none focus:ring-fuchsia-500 focus:border-fuchsia-500 bg-gray-600 border-gray-500 placeholder-gray-400 text-white"
            ...
            <div>
              <label for="phone" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Numéro de
                téléphone</label>
              <div :class="{ hidden: !showCountrySelector }" class="fixed z-9 top-0 left-0 w-full h-full"
                @click="showCountrySelector = false"></div>
              <div class="flex relative">
                <button v-if="selectedCountry" @click="() => (showCountrySelector = !showCountrySelector)"
                ...
                <button v-else @click="() => (showCountrySelector = !showCountrySelector)"
                  class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center border rounded-l-lg focus:outline-none bg-gray-700 hover:bg-gray-600 focus:ring-gray-700 text-white border-gray-600"
                  type="button">
                  --
                  <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                ...
                <div :class="{ hidden: !showCountrySelector }"
                  ...
                  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="states-button">
                    <li v-for="country in countries">
                      <button type="button" @click="() => (
                        ...
                        <img :src="`https://cdn.kcak11.com/CountryFlags/countries/${country.isoCode.toLowerCase()}.svg`"
                ...
                <input :disabled="!selectedCountry"
                  ...
                  v-model="Order.phone" ref="phoneRef" type="text" name="phone" id="phone" required />
              </div>
            </div>
  ...
  <section class="relative flex items-center justify-center h-fit mb-12 mt-16 md:mt-0 md:h-screen overflow-hidden">
  ...
  <section class="bg-gradient-to-b from-indigo-900 to-fuchsia-950 text-white p-6 text-center">
    <h3 class="text-3xl">
  ...
  <section id="abonnement" class="bg-black flex flex-col gap-4 py-12 px-4 text-white">
    <h3 class="mx-auto text-4xl md:text-6xl mb-6 text-center font-bold">
      Plans d'abonnement
    </h3>
    <div class="grid grid-cols-1 gap-4" :class="`lg:grid-cols-${Settings.plans.length}`">
      <div v-for="plan in Settings.plans" class="border-4 px-12 space-y-6" :class="plan.mois === 12
        ? 'relative bg-gradient-to-t from-fuchsia-900 to-transparent border-fuchsia-900 pt-6 pb-12'
        : 'border-gray-600 py-12'
        ">
        <span v-if="plan.mois === 12"
          class="bg-fuchsia-900 px-4 py-2 mx-auto text-center w-fit absolute top-0 right-0 left-0 rounded-b-3xl">lE PLUS
          VENDU</span>
        <h4 v-if="plan.prix === 0" class="text-2xl font-bold mx-auto text-center">
          24 Heures<br />GRATUIT
        </h4>
        <h4 v-else class="text-2xl font-bold mx-auto text-center">
          {{ plan.mois }} Mois<br />€{{ plan.prix }}
        </h4>
        <span class="block w-9 border-2 border-gray-600 mx-auto text-center"></span>
        ...
        <button @click="
          ...
          " :disabled="LoadingButton[`${plan.mois}-${plan.prix}`]" :class="LoadingButton[`${plan.mois}-${plan.prix}`]
          ...
          <svg v-if="LoadingButton[`${plan.mois}-${plan.prix}`]" aria-hidden="true" role="status"
            class="inline mr-2 w-8 h-8 animate-spin text-gray-200" viewBox="0 0 100 101" fill="none"
...
const Order = ref<OrderT>({});
const Message = ref();
const Loading = ref(false);
const LoadingButton = ref<Record<string, boolean>>({});
const FormCommander = ref();
import type { PayPalNamespace } from "@paypal/paypal-js";

const Commander = async (event?: Event) => {
	if (event) event.preventDefault();
	if (!Order.value || (Order.value.prix !== 0 && !Order.value.transaction))
		return;
	Loading.value = true;
	await $fetch("https://api.inicontent.com/iptv_pro/orders", {
		method: "POST",
		body: Order.value,
	});
	Loading.value = false;
...
const PaypalButton = () => {
	...
	if (paypalBtn) paypalBtn.innerHTML = "";
	...
	if (typeof Order.value.prix === "undefined") return;
						...
						purchase_units: [
							{
								amount: {
									currency_code: "EUR",
									value: Order.value.prix ? String(Order.value.prix) : "0",
								},
							},
						],
					});
...
watch(selectedCountry, (v) => {
	Order.value.phone = v.dialCode;
	Order.value.country = v.name;
	phoneRef.value.placeholder = `${v.dialCode}.......`;
